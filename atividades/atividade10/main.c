...
typedef struct {
    ...
    float temperatura_alarme;
    estado_sistema_t estado_sistema;
} dados_monitoramento_t;

QueueHandle_t fila_dados_monitoramento;
...
float ler_e_processar_temperatura() {
...
void sinalizar_proximidade_leds(float temp, float alarme, estado_sistema_t estado) {
...
void controlar_buzzer_pwm(float temp_atual, float temp_alarme, estado_sistema_t estado) {
    if (estado == ESTADO_ALARME_ATIVO) {
...
void atualizar_display_lcd(float temp_atual, float temp_alarme) {
    char linha1[20];
    char linha2[20];

    lcd_clear(&display);

    snprintf(linha1, sizeof(linha1), "Temp: %.1f C", temp_atual);
    snprintf(linha2, sizeof(linha2), "Alarme: %.1f C", temp_alarme);
...
void gravar_no_sdcard(float temp_atual, float temp_alarme, estado_sistema_t estado) {
    FILE *f = fopen(LOG_FILE_PATH, "a");
    if (f == NULL) {
        ESP_LOGE(SD_TAG, "Falha ao abrir o arquivo para escrita.");
        return;
    }
    ...
    fprintf(f, "Tempo: %.2f s, Temp: %.1f C, Alarme: %.1f C, Estado: %d\n",
            time_in_seconds, temp_atual, temp_alarme, estado);
    fclose(f);
}
...
void tarefa_leitura_ntc(void *pvParameters) {
    TickType_t xLastWakeTime;
    const TickType_t xFrequency = pdMS_TO_TICKS(INTERVALO_LEITURA_MS);
    xLastWakeTime = xTaskGetTickCount();

    float temperatura_atual;
    dados_monitoramento_t dados_envio;

    while (1) {
        temperatura_atual = ler_e_processar_temperatura();

        // Lógica de transição de estado da máquina de estados, movida da antiga tarefa_principal
        if (estado_atual == ESTADO_MONITORANDO && temperatura_atual >= temperatura_alarme) {
        ...
        } else if (estado_atual == ESTADO_ALARME_ATIVO && temperatura_atual < temperatura_alarme) {
        ...
        } else if (estado_atual == ESTADO_AJUSTANDO_ALARME) {
        ...
        dados_envio.temperatura_ntc = temperatura_atual;
        dados_envio.temperatura_alarme = temperatura_alarme;
        dados_envio.estado_sistema = estado_atual;

        if (xQueueSend(fila_dados_monitoramento, &dados_envio, 0) != pdPASS) {
            ESP_LOGW(APP_TAG, "Fila de dados de monitoramento cheia!");
        }
...
void tarefa_controle_buzzer(void *pvParameters) {
    dados_monitoramento_t dados_recebidos;
    while (1) {
        if (xQueueReceive(fila_dados_monitoramento, &dados_recebidos, portMAX_DELAY) == pdPASS) {
            controlar_buzzer_pwm(dados_recebidos.temperatura_ntc, dados_recebidos.temperatura_alarme, dados_recebidos.estado_sistema);
        }
    }
}

void tarefa_atualizar_lcd(void *pvParameters) {
    dados_monitoramento_t dados_recebidos;
    while (1) {
        if (xQueueReceive(fila_dados_monitoramento, &dados_recebidos, portMAX_DELAY) == pdPASS) {
            atualizar_display_lcd(dados_recebidos.temperatura_ntc, dados_recebidos.temperatura_alarme);
        }
    }
}

void tarefa_sinalizacao_display(void *pvParameters) {
    dados_monitoramento_t dados_recebidos;
    while (1) {
        if (xQueueReceive(fila_dados_monitoramento, &dados_recebidos, portMAX_DELAY) == pdPASS) {
            sinalizar_proximidade_leds(dados_recebidos.temperatura_ntc, dados_recebidos.temperatura_alarme, dados_recebidos.estado_sistema);
        }
        // Pequeno atraso para o piscar do LED "F" no estado de alarme ativo
        if (dados_recebidos.estado_sistema == ESTADO_ALARME_ATIVO) {
            vTaskDelay(pdMS_TO_TICKS(500));
        }
    }
}

void tarefa_log_sdcard(void *pvParameters) {
    dados_monitoramento_t dados_recebidos;
    TickType_t xLastWakeTime;
    const TickType_t xFrequency = pdMS_TO_TICKS(INTERVALO_LEITURA_MS * 5);
    xLastWakeTime = xTaskGetTickCount();

    while (1) {
        if (xQueueReceive(fila_dados_monitoramento, &dados_recebidos, xFrequency) == pdPASS) {
             gravar_no_sdcard(dados_recebidos.temperatura_ntc, dados_recebidos.temperatura_alarme, dados_recebidos.estado_sistema);
        } else {
             // Caso a fila esteja vazia no momento, grava o último dado válido ou um placeholder
             // Para esta implementação, vamos assumir que dados_recebidos ainda mantém o último valor
             // recebido ou um valor default se nunca recebeu
             gravar_no_sdcard(dados_recebidos.temperatura_ntc, dados_recebidos.temperatura_alarme, dados_recebidos.estado_sistema);
        }
        vTaskDelayUntil(&xLastWakeTime, xFrequency);
    }
...
void app_main() {
    ...
    fila_dados_monitoramento = xQueueCreate(1, sizeof(dados_monitoramento_t));
    if (fila_dados_monitoramento == NULL) {
        ESP_LOGE(APP_TAG, "Falha ao criar a fila de dados de monitoramento. Abortando...");
        return;
    } else {
        ESP_LOGI(APP_TAG, "Fila de dados de monitoramento criada com sucesso.");
    }

    xTaskCreate(tarefa_controle_botoes, "Controle_Botoes", 2048, NULL, 10, NULL);
    xTaskCreate(tarefa_leitura_ntc, "Leitura_NTC", 4096, NULL, 5, NULL);
    xTaskCreate(tarefa_controle_buzzer, "Controle_Buzzer", 2048, NULL, 6, NULL);
    xTaskCreate(tarefa_atualizar_lcd, "Atualizar_LCD", 4096, NULL, 4, NULL);
    xTaskCreate(tarefa_sinalizacao_display, "Sinalizacao_Display", 2048, NULL, 4, NULL);
    xTaskCreate(tarefa_log_sdcard, "Log_SDCard", 4096, NULL, 3, NULL);
}
